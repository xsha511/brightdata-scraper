  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      try {
        ...
        // ========== 提取所有图表数据 ==========
        const allChartData = { total_sales: [], daily_sales: [], revenue: [], price: [] };
        let chartHistoryData = [];  // 当前图表数据
        ...
        try {
          ...
          if (chartNodeId) {
            ...
            if (boxModel.model) {
              ...
              // ===== 保存第一个图表数据（总销量） =====
              allChartData.total_sales = [...chartHistoryData];
              chartDebugInfo.charts.total_sales = chartHistoryData.length;
              console.log('[Debugger] 总销量 extracted:', chartHistoryData.length, 'points');

              // ===== 提取其他3个图表 =====
              const otherCharts = [
                { key: 'daily_sales', label: '日销量', index: 1 },
                { key: 'revenue', label: '销售额', index: 2 },
                { key: 'price', label: '价格', index: 3 }
              ];
              ...
              if (tabSearch.resultCount >= 4) {
                ...
                const tabNodeIds = tabNodes.nodeIds;
                ...
                for (const chart of otherCharts) {
                  try {
                    console.log(`[Debugger] Extracting ${chart.label}...`);

                    // 点击标签
                    const tabBoxModel = await chrome.debugger.sendCommand({ tabId }, 'DOM.getBoxModel', { nodeId: tabNodeIds[chart.index] });
                    if (tabBoxModel.model) {
                      ...
                      for (let i = 0; i < numSamples; i++) {
                        ...
                        // 查找 tooltip
                        for (const query of tooltipQueries) {
                          try {
                            const search = await chrome.debugger.sendCommand({ tabId }, 'DOM.performSearch', {
                            ...
                            if (search.resultCount > 0) {
                              ...
                              for (const nid of results.nodeIds) {
                                try {
                                  const htmlResult = await chrome.debugger.sendCommand({ tabId }, 'DOM.getOuterHTML', { nodeId: nid });
                                  const text = (htmlResult.outerHTML || '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
                                  if (text && text.length > 5 && text.length < 300 && /\d{1,2}[-\/]\d{1,2}/.test(text)) {
                                    if (!thisChartData.some(d => d.text === text)) {
                                      const dateMatch = text.match(/(\d{1,2}[-\/]\d{1,2})/);
                                      const valueMatch = text.match(/[：:]\s*([\d,.]+)/) || text.match(/([\d,.]+)\s*$/);
                                      thisChartData.push({
                                        text: text,
                                        date: dateMatch ? dateMatch[1] : null,
                                        value: valueMatch ? valueMatch[1].replace(/,/g, '') : null
                      ...
                      allChartData[chart.key] = thisChartData;
                      chartDebugInfo.charts[chart.key] = thisChartData.length;
                      console.log(`[Debugger] ${chart.label} extracted:`, thisChartData.length, 'points');
                    }
                  } catch (e) {
                    console.log(`[Debugger] Failed to extract ${chart.label}:`, e.message);
                    chartDebugInfo.charts[chart.key] = { error: e.message };
                  }
                }
              }
            }
          ...
          const totalPoints = Object.values(allChartData).reduce((sum, arr) => sum + (Array.isArray(arr) ? arr.length : 0), 0);
